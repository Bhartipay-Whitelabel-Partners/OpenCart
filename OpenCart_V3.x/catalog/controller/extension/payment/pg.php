...
class ControllerExtensionPaymentPG extends Controller
{
    ...
    public function index()
    {
        ...
        if (!$this->config->get('payment_pg_test')) {
            $data['action'] = 'https://merchant.pg.com/crm/jsp/paymentrequest';
        } else {
            $data['action'] = 'http://uat.pg.com/crm/jsp/paymentrequest';
        }
        ...
        $return_url = $this->url->link('extension/payment/pg/callback', 'language=' . $this->config->get('config_language') . '&hash=' . md5($order_info['order_id'] . $order_info['total'] . $order_info['currency_code'] . $this->config->get('payment_pg_salt')));

        $transaction_request = new BPPGModule();

        /* Setting all values here */
        $transaction_request->setPayId($this->config->get('payment_pg_pay_id'));
        $transaction_request->setPgRequestUrl($data['action']);
        $transaction_request->setSalt($this->config->get('payment_pg_salt'));
        ...
        return $this->load->view('extension/payment/pg', $postdata);
    }
    ...
    public function callback()
    {
        ...
        if ($order_info) {
            ...
            } elseif ($this->request->get['hash'] != md5($order_info['order_id'] . $order_info['total'] . $order_info['currency_code'] . $this->config->get('payment_pg_salt'))) {
                $error = $this->language->get('text_unable');
            }
        } else {
            $error = $this->language->get('text_unable');
        ...
        if ($error) {
            ...
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_pg_order_status_id'));
