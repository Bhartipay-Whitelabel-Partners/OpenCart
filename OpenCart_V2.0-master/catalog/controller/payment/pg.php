...
class ControllerPaymentPG extends Controller
{
    ...
    public function index()
    {
        ...
        if (!$this->config->get('PG_test')) {
            $data['action'] = 'https://merchant.PG.com/crm/jsp/paymentrequest';
        } else {
        ...
        $return_url = $this->url->link('payment/PG/callback', 'language=' . $this->config->get('config_language') . '&hash=' . md5($order_info['order_id'] . $order_info['total'] . $order_info['currency_code'] . $this->config->get('PG_salt')));

        $transaction_request = new PGPGModule();

        /* Setting all values here */
        $transaction_request->setPayId($this->config->get('PG_pay_id'));
        $transaction_request->setPgRequestUrl($data['action']);
        $transaction_request->setSalt($this->config->get('PG_salt'));
        ...
        return $this->load->view('default/template/payment/PG.tpl', $postdata);
    }
    ...
    public function callback()
    {
		...
		$data['text_title']            = $this->language->get('Credit Card / Debit Card (PG)');
		$data['text_unable']           = $this->language->get('Unable to locate or update your order status');
		$data['text_declined']         = $this->language->get('Payment was declined by PG');
		$data['text_failed']           = $this->language->get('PG Transaction Failed');
		$data['text_failed_message']   = $this->language->get('<p>Unfortunately there was an error processing your PG transaction.</p><p><b>Warning: </b>%s</p><p>Please verify your PG account balance before attempting to re-process this order</p><p> If you believe this transaction has completed successfully, or is showing as a deduction in your PG account, please <a href="%s">Contact Us</a> with your order details.</p>');
        ...
        if ($order_info) {
            ...
            } elseif ($this->request->get['hash'] != md5($order_info['order_id'] . $order_info['total'] . $order_info['currency_code'] . $this->config->get('PG_salt'))) {
                $error = $this->language->get('text_unable');
            }
        } else {
            $error = $this->language->get('text_unable');
        ...
        if ($error) {
            ...
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('PG_order_status_id'));
